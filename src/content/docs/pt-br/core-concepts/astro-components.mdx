...
Para passar múltiplos elementos HTML no placeholder `<slot/>` de um component sem encapsulá-los em uma `<div>`, use o atributo `slot=""` no [componente `<Fragment/>` do Astro](/pt-br/reference/api-reference/#fragment-):

```astro title="src/components/CustomTable.astro" "<slot name="header"/>" "<slot name="body"/>"
---
// Cria uma tabela customizada com slots nomeados como placeholders para o conteúdo do corpo e cabeçalho
---
...
Injeto múltiplas linhas e colunas de conteúdo HTML utilizando um atributo `slot=""` para especificar o conteúdo do cabeçalho (`"header"`) e do corpo (`"body"`). Elementos HTML individuais também podem ser estilizados:
...
---
<CustomTable>
  <Fragment slot="header"> <!-- passe o cabeçalho da tabela -->
    <tr><th>Nome do produto</th><th>Unidades em estoque</th></tr>
  </Fragment>

  <Fragment slot="body"> <!-- passe o corpo da tabela -->
    <tr><td>Sandálias</td><td>64</td></tr>
...
:::note
O nome de um slot do Astro não pode ser gerado dinamicamente, como dentro de uma função `map`. Se essa funcionalidade for necessária dentro de um componente de framework de UI, pode ser melhor gerar esses slots dinâmicos dentro do próprio framework.
:::
...
```astro title="src/layouts/LayoutBase.astro" {9,12}
---
---
<html lang="pt-BR">
	<head>
